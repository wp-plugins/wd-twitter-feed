...
abstract class TwitterResource {
	...
	protected $settings;
	protected $request_count;	private $debug;	private $twitter_feed_options;
	private $options;	const API_URL = 'https://api.twitter.com/1.1/';
	const USER_TIMELINE = 'statuses/user_timeline.json';
	const HOME_TIMELINE = 'statuses/home_timeline.json';
	const MENTIONS_TIMELINE = 'statuses/mentions_timeline.json';
	const RETWEETS_OF_ME = 'statuses/retweets_of_me.json';
	const LISTS = 'lists/statuses.json';
	...
	public function __construct($settings) {
		...
		$this->settings = $settings;		$this->twitter_feed_options = TwitterFeedOptions::get_instance();
		$this->options = get_option( $this->twitter_feed_options->get_option_name() );		$this->debug = Debug::get_instance();
		$this->debug->append('New ' . $this->settings['resource'] . ' resource created');		$this->init();
	}
	 ...
	 */
	public function perform_request($tokens) {		if(!isset($this->initial_count)) $this->initial_count = $this->settings['count'];		$this->debug->append('Performing a ' . $this->settings['resource'] . ' type request');
		$this->debug->append('Initial count: ' . $this->initial_count);
		$this->debug->append('arguments: ' . $this->build_argument_list());		$twitter = new TwitterAPIExchange($tokens);
		$resp = $twitter->setGetfield($this->build_argument_list())
						->buildOauth($this->get_url(), 'GET')
						->performRequest();		$this->increment_request_count();		$tweets = $this->filter_response(json_decode($resp));		$this->debug->append('Received count: ' . count($tweets));		if(
			count($tweets) < $this->initial_count && 
			$this->options['force_tweet_count']['enable'] && 
			$this->get_request_count() < $this->options['force_tweet_count']['request_limit']
		) {
