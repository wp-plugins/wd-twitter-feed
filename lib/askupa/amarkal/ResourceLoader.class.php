	...
	public function getFilesByExt($ext) {
		...
		switch($ext) {
			...
			case self::PHP_CLASSES:
				...
				break;
			case self::KRG_CLASSES:
				return $fileTree['krg']['classes'];
				break;
			case self::KRG_FILES:
				return $fileTree['krg']['classes'];
				break;
			...
			foreach($it as $file) {
				...
				switch($fileinfo['extension']) {
					...
					case 'php':
						...
						// KRG classes (file.krg.php)
						else if(substr($fileinfo['filename'], -4) === '.krg') {
							$files['krg']['classes'][] = array(
								'name' => str_replace('.krg', '', $fileinfo['filename']),
								'path' => $file->getPathname()
							);
						}
