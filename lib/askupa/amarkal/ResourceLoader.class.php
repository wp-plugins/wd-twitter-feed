 ...
 * project.
 */
class ResourceLoader {	private $basedir;
	private $fileTree;
	...
	private $phpClasses;
	private $phpFiles;	const PHP_CLASSES = 0;
	const PHP_FILES = 1;
	const JS = 2;
	const CSS = 3;
	...
	public function autoload($class) {
		$class = ltrim($class, '\\');
		$files = $this->getFilesByExt(self::PHP_CLASSES);		if(strpos($class, __NAMESPACE__) !== 0)
			return;
		...
		foreach($files as $file)
			if($file['name'] === str_replace('\\', '', $class))
				$path = $file['path'];
		
		if(!file_exists($path)) {
			echo 'The following class could not be loaded: ' . __NAMESPACE__ . $class . ' ';
			echo 'Path: ' . $path;
		}
	 ...
	 *					types
	 */
	public function getFileTree() {		if(!isset($this->fileTree)) {
			
			$di = new \RecursiveDirectoryIterator(PLUGIN_DIR,\RecursiveDirectoryIterator::SKIP_DOTS);
			$it = new \RecursiveIteratorIterator($di);			$files = array();
			foreach($it as $file) {				$fileinfo = pathinfo($file);
				...
				switch($fileinfo['extension']) {					case 'php':						if(substr($fileinfo['filename'], -6) === '.class') {
	 ...
	 */
	function get_file_data( $file, $default_headers, $context = '' ) {		$fp = fopen( $file, 'r' );		$file_data = fread( $fp, 1024 );		fclose( $fp );		$file_data = str_replace( "\r", "\n", $file_data );
		...
		foreach ( $all_headers as $field => $regex ) {
			if ( preg_match( '/^[ \t\/*#@]*' . preg_quote( $regex, '/' ) . ':(.*)$/mi', $file_data, $match ) && $match[1] ) {
				$value = _cleanup_header_comment( $match[1] );				if(preg_match('/{([^}]*)}/', $value, $arr))
