...
namespace TWITTERFEED;require_once('ResourceLoader.class.php');
...
abstract class Amarkal {
	...
	/* Static variables */	private static $_instances = array();	protected function __construct() {}
	
	/**
	 * Initiate
	 */
	protected function init() {		$this->resource_loader = new ResourceLoader();
		$this->resource_loader->registerAutoloadFunc();		register_activation_hook( __FILE__, array( $this, 'activate' ) );
		register_deactivation_hook( __FILE__, array( $this, 'deactivate' ) );		add_action( 'init', array( $this, 'load_plugin_textdomain' ) );		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_admin_styles' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_admin_scripts' ) );
		add_action( 'admin_enqueue_scripts', array( $this, 'localize_script' ) );		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_public_styles' ) );
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_public_scripts' ) );
		add_action( 'wp_enqueue_scripts', array( $this, 'localize_script' ) );		add_action( 'wp_ajax_'.PLUGIN_CLASS.'_thickbox_callback', array($this, 'thickbox_callback') );
		add_action( 'wp_ajax_'.PLUGIN_CLASS.'_menu_items', array($this, 'get_tinymce_menu_items') );
	}
	 ...
	 * @since 1.0
	 */
	public static function get_instance($basedir = null) {
        // Get the child class
		$class = get_called_class();        if (!isset(self::$_instances[$class])) {
			self::generate_defines($basedir);
	 ...
	 * @since 1.0 
	 */
	public static function generate_defines($basedir) {		$headers = get_file_data(
			...
			'PLUGIN_CLASS' => preg_replace('/\s+/', '', strtolower($headers['name'])),
		...
		self::fstream($dir, 'w+', $basedir);		foreach($defines as $name => $value)
	 ...
	 * 
	 */
	public abstract function get_tinymce_menu_items();
}